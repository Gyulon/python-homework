...
# 分析年龄分布比例（曲线图）
age_distribution = sns.kdeplot(student['age'], fill=True)
...
# 分性别年龄分布图（柱状图）
age_distribution_sex = sns.countplot(x='age', hue='sex', data=student)
...
# 各年龄段的成绩分布图##########################################
age_grade_plot = sns.stripplot(x='age', y='G3', data=student, size=4, jitter=True)
age_grade_plot.axes.set_title('年龄与分数', fontsize=30)
age_grade_plot.set_xlabel('年龄', fontsize=20)
age_grade_plot.set_ylabel('分数', fontsize=20)
plt.show()
...
sns.kdeplot(student.loc[student['address'] == 'U', 'G3'], label='Urban', fill=True)
sns.kdeplot(student.loc[student['address'] == 'R', 'G3'], label='Rural', fill=True)
...
# 失败次数成绩分布图################################################
failures_plot = sns.stripplot(x='failures', y='G3', data=student, size=4, jitter=True)
failures_plot.axes.set_title('失败次数少的学生分数更高吗？', fontsize=30)
failures_plot.set_xlabel('失败次数', fontsize=20)
failures_plot.set_ylabel('最终成绩', fontsize=20)
plt.show()
